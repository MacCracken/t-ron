...
impl RiskScorer {
    ...
    pub async fn score(audit: &AuditLogger, agent_id: &str) -> f64 {
        ...
        let denials = events.iter().filter(|e| e.verdict == "deny").count() as f64;
        let flags = events.iter().filter(|e| e.verdict == "flag").count() as f64;
...
mod tests {
    use super::*;
    use crate::gate::{ToolCall, Verdict, DenyCode};
    ...
    async fn all_denied_max_risk() {
        ...
        for _ in 0..10 {
            audit.log(&call, &Verdict::Deny { reason: "nope".into(), code: DenyCode::Unauthorized }).await;
        }
